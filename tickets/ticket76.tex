...
\begin{theorem-non}
    $f, g:<a,b> \rightarrow \mathbb{R}$ $n$ раз дифф. в точке $x \in <a,b>$. Тогда:

    \begin{enumerate}
        \item $\alpha f + \beta g$ $n$ раз дифф. в $(.) \ x$ и $(\alpha f + \beta g)^{(n)} (x) = \alpha f^{(n)}(x) + \beta g^{(n)}(x)$ 
        \item $fg$ $n$ раз дифф. в $(.) \ x$ и $(fg)^{(n)}(x) = \sum\limits_{k=0}^{n} C_n^k f^{(k)}(x) g^{(n-k)} (x)$
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        Заменим $j= k+1$ и вытащим крайние слагаемые: $f^{(n+1)} + \sum\limits_{j=1}^{n} C_n^{j-1} f^{(j)} g^{(n+1-j)} + \sum\limits_{j=1}^{n} C_n^j f^{(j)} g^{(n+1-j)} + g^{(n+1)}$\\
        Так как из комбинаторики $C_n^j + C_n^{j-1} = C_{n+1}^j$: $f^{(n+1)} + \sum\limits_{j=1}^{n} C_{n+1}^j f^{(j)} g^{(n+1-j)} + g^{(n+1)}$\\
        $= \sum\limits_{j=0}^{n+1} C_n^j f^{(j)}(x) g^{(n+1-j)} (x)$

        \item Индукция: $n \longrightarrow n+1$
        
        $(f(\alpha x + \beta))^{(n+1)} = ((f(\alpha x + \beta))^{(n)})' = (\alpha^n f^{(n)}(\alpha x + \beta))' = 
        \alpha^{n+1} f^{(n+1)}(\alpha x + \beta)$
...
\begin{enumerate}
    ...
    \item $(\sin x)^(n) = sin(x + \pi/2)$
    \item $(\cos x)^(n) = cos(x + \pi/2)$
\end{enumerate}
